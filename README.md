...
- **Walkthrough:** Deposit, Shield, Execute and Withdraw explain the journey in four selectable stages. A short sticky sequence is used only when the board fits the viewport. Step selection and Replay navigate to the matching stage; reduced motion keeps manual selection available. Wheel and touch scrolling are never captured.
- **Continuous brand story:** a line leaves the institution's roof, splits at the page edge, and turns into the two slashes in the shielded-association diagram. It assembles into the official mark and rests inside the Shield step while a quiet underline indicates the current explanation. It then docks inside the API box and finally joins “Start trading with ShieldTX.” Open FAQ items carry a small borderless mark. The sequence follows native scrolling in both directions; reduced motion retains static marks and a canvas fallback works without WebGL.
